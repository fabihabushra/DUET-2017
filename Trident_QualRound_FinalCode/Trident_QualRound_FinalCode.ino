...
if  (fDistance < 20 && obsKey == 0 && fDistance != 4) 
{ 
...
{     
      ...
      //  findPaths(); //turned off findpath for black
}
...
{  
  ...
  if (sensorValue == 0b11100000 || sensorValue == 0b11110000) 
    { mappedValue = 80; plannedCRotate(); while((mappedValue<-60)||(mappedValue>60)){sensorRead(); sensorMapping();}}         
  ...
  //90 deg left turn
  else if (sensorValue == 0b00000111 || sensorValue == 0b00001111) {mappedValue = -80; plannedACRotate(); while((mappedValue<-60)||(mappedValue>60)){sensorRead(); sensorMapping();}}   
